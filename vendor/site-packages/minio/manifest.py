...
addon_directory = "/tmp/SuluRender"
...
with zipfile.ZipFile(extension_path, "w") as extension_archive:
    for root, dirs, files in os.walk(addon_directory):
        for file in files:
            ...
            if any(
                excluded_file in file_path for excluded_file in exclude_files_extension
            ):
                continue
            else:
...
with open(extension_index, "w") as f:
    ...
    index["data"][0]["archive_url"] = (
        f"https://github.com/Superluminal/blender-addon/releases/download/{version}/SuluRender.zip"
    )
    index["data"][0]["archive_size"] = len(archive_content)
    index["data"][0]["archive_hash"] = (
        f"sha256:{hashlib.sha256(archive_content).hexdigest()}"
    )
