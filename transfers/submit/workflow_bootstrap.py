...
def resolve_bootstrap_deps(*, pkg_name: str, set_logger_fn, log_fn) -> BootstrapDeps:
    ...
    def _log_upload_result_wrapper(result, expected_bytes: int = 0, label: str = "") -> None:
        workflow_runtime_helpers.log_upload_result(
            result,
    ...
    def _check_rclone_errors_wrapper(result, label: str = "") -> None:
        workflow_runtime_helpers.check_rclone_errors(
            result,
            label=label,
            debug_enabled_fn=worker_utils.debug_enabled,
            log_fn=log_fn,
    ...
    upload_deps = UploadDeps(
        ...
        log_fn=log_fn,
        format_size_fn=worker_utils.format_size,
        rclone_bytes_fn=workflow_runtime_helpers.rclone_bytes,
        rclone_stats_fn=workflow_runtime_helpers.rclone_stats,
        is_empty_upload_fn=workflow_runtime_helpers.is_empty_upload,
        get_rclone_tail_fn=workflow_runtime_helpers.get_rclone_tail,
        log_upload_result_fn=_log_upload_result_wrapper,
        check_rclone_errors_fn=_check_rclone_errors_wrapper,
        is_filesystem_root_fn=workflow_runtime_helpers.is_filesystem_root,
