...
from .errors import CancelledError, TransportError
...
def _http_status_error(exc: urllib.error.HTTPError, *, operation: str) -> TransportError:
    ...
    if exc.code == 426:
        return TransportError(
            "Update the Sulu Market Bridge or Blender; the server rejected this runtime version"
        )
    if 300 <= exc.code < 400:
        ...
        *,
        ...
        try:
            with self._opener.open(request, timeout=self.timeout_seconds) as response:
                unregister = (
                    cancellation.register(response.close) if cancellation is not None else None
                )
                try:
                    if cancellation is not None:
                        cancellation.raise_if_cancelled()
                    if response.status != 200:
                        raise TransportError(
                            "Sulu Market returned an unexpected redemption status"
                        )
                    content_type = response.headers.get_content_type()
                    if content_type != "application/json":
                        raise TransportError(
                            "Sulu Market returned an invalid redemption content type"
                        )
                    raw = _read_bounded(
                        response,
                        maximum=REDEEM_RESPONSE_MAX_BYTES,
            ...
            if cancellation is not None and cancellation.cancelled:
                exc.close()
                raise CancelledError("Sulu Market asset import was cancelled") from exc
        ...
        grant = parse_redeem_response(raw, max_artifact_bytes=self.max_artifact_bytes)
        validate_runtime_compatibility(
            grant.compatibility,
            bridge_version=CLIENT_VERSION,
            blender_version=self.blender_version,
        )
        if grant.compatibility != descriptor.compatibility:
            raise TransportError(
                "Sulu Market changed compatibility requirements during ticket redemption"
            )
        return grant
        ...
        try:
            with self._opener.open(request, timeout=self.timeout_seconds) as response:
                ...
                try:
                    ...
                    if response.status != 200:
                        raise TransportError(
                            "Sulu Market returned an unexpected download status"
                        )
                    content_type = response.headers.get_content_type()
